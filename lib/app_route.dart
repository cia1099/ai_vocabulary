...
class AppRoute<T> extends PageRoute<T> {
  ...
  Widget routeBuilder(BuildContext context) {
    ...
    if (!validation(context)) {
      if (currentWord == null && StudyRouters.asMap().values.contains(path)) {
  ...
  // @override
  // Widget buildContent(BuildContext context) => routeBuilder(context);

  // @override
  // String? get title =>
  //     Uri.tryParse(settings.name ?? '')?.pathSegments.lastOrNull;
}
